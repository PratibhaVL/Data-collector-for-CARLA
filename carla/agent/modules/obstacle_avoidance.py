...
class ObstacleAvoidance(object):
    ...
    def __init__(self, param,param_controller,city_name):
    ...
    def stop_for_agents(self, location, orientation, wp_angle, wp_vector, agents):
        ...
        for agent in agents:
            if agent.HasField('traffic_light') and self.param['stop4TL']:
            ...
            elif agent.HasField('vehicle') and self.param['stop4V']:
                ...
                if self.is_speed_limit_sign_visible(location , agent):
                    self.param_controller['target_speed'] = float(round(agent.speed_limit_sign.speed_limit , 2)) * 3.6
                    #active_agents_ids.append(agent.id)
            speed_factor = min(speed_factor_tl, speed_factor_p, speed_factor_v)
            
                
        state = {
            'speed_limit': self.param_controller['target_speed'],
    ...
    """ **********************
        ...
        **********************
    """
    def is_speed_limit_sign_visible(self, location, agent):
